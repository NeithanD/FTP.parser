...
class ImagesImport extends Base
{
    ...
    protected function deleteTempFiles(){
        ...
        if ($this->connect())
        {
        $arDelete = \ftp_nlist($this->connect, "/www/ooobalf.ru/testimport/images/tmp/");
            
            foreach($arDelete as $deleteFile){
                unlink("/var/www/bx557/data".$deleteFile);
            }
            ...
            if (!empty($arFiles))
            {
                foreach ($arFiles as $file)
                {
                    ...
                    if (in_array($ext, ['jpg', 'jpeg', 'png']))
                    {
                     ...
                     $article = str_replace('/www/ooobalf.ru/web/', '', $bufName[0]);
    ...
    protected function getFile($file)
    {
        ...
        $name =  str_replace('/www/ooobalf.ru/web/', '', $file);
        if ($this->connect())
        {
            ...
            echo "проблема с открытием файла ". $file;
            }
        ...
        if (!empty($arFiles))
        {
            ...
            foreach($arFiles as $key => $arProp){
                ...
                if(is_int(strpos($arProp[0], '_'))){
                    ...
                    if(str_replace('/www/ooobalf.ru/web/', '', substr($arProp[0], 0, -6)) == $moreArticle){
                     
                        
                            $PROPERTY_VALUE[] = [
                                
                                'VALUE' => \CFile::MakeFileArray(str_replace('/www/ooobalf.ru', '', $arProp[0])),
                                'DESCRIPTION' => ""
                                            ];
